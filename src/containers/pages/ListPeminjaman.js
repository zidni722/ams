...
const Listpeminjaman = ({ id, code, title, category, tenant, createDate, verificationDate, status, statusColor }) => {
  
  return (
    
    <Card onClick = {() => window.location.href="/app/menu-peminjaman/detail-peminjaman"} className="btn-shadow d-flex flex-row mb-3">
      <div className="d-flex flex-grow-1 min-width-zero">
...
const ListItemPeminjaman = () => {
  return (
    <Row>
      <Colxx>
      <Title/>
        {products.map((products, index) => {
          return <Listpeminjaman key={`products_${index}`} {...products} />;
        })}
      </Colxx>
    </Row>
  );
...
export default ListItemPeminjaman;
